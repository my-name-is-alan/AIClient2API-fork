...
function hasConfiguredProviderPool(config, providerType) {
    const configuredPool = config.providerPools?.[providerType];
    return Array.isArray(configuredPool) && configuredPool.length > 0;
}
...
export async function getApiService(config, requestedModel = null, options = {}) {
    ...
    if (providerPoolManager && hasConfiguredProviderPool(config, config.MODEL_PROVIDER)) {
        // 如果有号池管理器，并且当前模型提供者类型有非空号池，则从号池中选择一个提供者配置
