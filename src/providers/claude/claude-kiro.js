...
const KIRO_CONSTANTS = {
    ...
    MANAGEMENT_URL: 'https://management.{{region}}.kiro.dev',
...
const KIRO_RUNTIME_REGIONS = new Set(['us-east-1', 'eu-central-1']);
const KIRO_Q_ENDPOINTS = {
    'us-east-1': 'https://q.us-east-1.amazonaws.com/generateAssistantResponse',
    'eu-central-1': 'https://q.eu-central-1.amazonaws.com/generateAssistantResponse',
    'us-gov-east-1': 'https://q-fips.us-gov-east-1.amazonaws.com/generateAssistantResponse',
    'us-gov-west-1': 'https://q-fips.us-gov-west-1.amazonaws.com/generateAssistantResponse',
    'us-iso-east-1': 'https://q.us-iso-east-1.c2s.ic.gov/generateAssistantResponse',
    'us-isob-east-1': 'https://q.us-isob-east-1.sc2s.sgov.gov/generateAssistantResponse',
    'us-isof-south-1': 'https://q.us-isof-south-1.csp.hci.ic.gov/generateAssistantResponse',
    'us-isof-east-1': 'https://q.us-isof-east-1.csp.hci.ic.gov/generateAssistantResponse'
};
...
function resolveKiroDefaultBaseUrl(region = 'us-east-1') {
    const normalizedRegion = region || 'us-east-1';
    if (KIRO_RUNTIME_REGIONS.has(normalizedRegion)) {
        return KIRO_CONSTANTS.BASE_RUNTIME_URL.replace("{{region}}", normalizedRegion);
    }
    if (KIRO_Q_ENDPOINTS[normalizedRegion]) {
        return KIRO_Q_ENDPOINTS[normalizedRegion];
    }
    return KIRO_CONSTANTS.BASE_URL.replace("{{region}}", normalizedRegion);
}
...
function shortenKiroToolName(name) {
    const rawName = sanitizeKiroToolName(name);
    if (rawName.length <= KIRO_MAX_TOOL_NAME_LENGTH) {
        return rawName;
    }

    const hash = crypto.createHash('sha256').update(rawName).digest('hex').slice(0, 12);
    const prefixLength = KIRO_MAX_TOOL_NAME_LENGTH - hash.length;
    return `${rawName.slice(0, prefixLength)}${hash}`;
}
...
function buildKiroToolNameMaps(tools) {
    ...
    if (Array.isArray(tools)) {
        for (const tool of tools) {
            ...
            let aliasName = shortenKiroToolName(originalName);
            const existingOriginalName = aliasToOriginal.get(aliasName);
            if (existingOriginalName && existingOriginalName !== originalName) {
                aliasCounts.set(aliasName, (aliasCounts.get(aliasName) || 1) + 1);
                const suffix = crypto.createHash('sha256').update(originalName).digest('hex').slice(0, 8);
                const prefixLength = KIRO_MAX_TOOL_NAME_LENGTH - suffix.length;
                aliasName = `${aliasName.slice(0, prefixLength)}${suffix}`;
            } else {
                aliasCounts.set(aliasName, 1);
            }
    ...
    return {
        aliasToOriginal,
        toKiroName: (name) => originalToAlias.get(name) || name,
        fromKiroName: (name) => aliasToOriginal.get(name) || name
    };
}

function normalizeKiroToolDescription(description, name) {
    const normalized = typeof description === 'string' ? description : String(description || '');
    const trimmed = normalized.trim();
    return trimmed ? normalized : `Tool: ${name}`;
}

function cloneKiroToolSchemaValue(value) {
    if (Array.isArray(value)) {
        return value.map(item => cloneKiroToolSchemaValue(item));
    }

    if (value && typeof value === 'object') {
        const cloned = {};
        for (const [key, nestedValue] of Object.entries(value)) {
            cloned[key] = cloneKiroToolSchemaValue(nestedValue);
        }
        return cloned;
    }
...
function cleanKiroToolSchemaObject(schema) {
    ...
    if (Array.isArray(schema.required)) {
        const requiredFields = schema.required.filter(field =>
            typeof field === 'string' &&
            field.trim() !== '' &&
            (!properties || Object.prototype.hasOwnProperty.call(properties, field))
        );
        ...
        if (Array.isArray(value)) {
            for (const item of value) {
                if (item && typeof item === 'object' && !Array.isArray(item)) {
                    cleanKiroToolSchemaObject(item);
                }
            }
        } else if (value && typeof value === 'object') {
            cleanKiroToolSchemaObject(value);
        }
    }
...
function sanitizeKiroToolSchema(schema) {
    if (!schema || typeof schema !== 'object' || Array.isArray(schema)) {
        return { type: 'object', properties: {} };
    }

    const cleaned = cleanKiroToolSchemaObject(cloneKiroToolSchemaValue(schema));
...
function createKiroEmptyResponseError(model, inputTokens = 0, options = {}) {
    const reason = options.reason || 'empty';
    const reasonMessage = reason === 'ignored_events'
        ? 'Only ignored Kiro side-channel events were received'
        : 'Empty response from upstream';
    const error = new Error(`[Kiro] ${reasonMessage} for model ${model || 'unknown'} (input_tokens=${inputTokens})`);
    error.name = 'KiroEmptyResponseError';
    error.code = 'KIRO_EMPTY_RESPONSE';
    error.status = 502;
    error.shouldSwitchCredential = true;
    error.emptyResponseReason = reason;
    if (reason === 'ignored_events') {
        error.skipErrorCount = true;
    }
    return error;
}

function createKiroIncompleteToolUseError(toolCall, reason) {
    const toolName = toolCall?.name || 'unknown';
    const toolUseId = toolCall?.toolUseId || 'unknown';
    const error = new Error(`[Kiro] Incomplete tool_use input for ${toolName} (${toolUseId}): ${reason}`);
    error.name = 'KiroIncompleteToolUseError';
    error.code = 'KIRO_TOOL_USE_INCOMPLETE';
    error.status = 502;
    error.shouldSwitchCredential = true;
    return error;
}
...
export function resolveKiroModelId(model) {
    return MODEL_MAPPING[model] || model;
}

const KIRO_AUTH_TOKEN_FILE = "kiro-auth-token.json";
const KIRO_PROFILE_FILE = path.join(os.homedir(), "AppData", "Roaming", "Kiro", "User", "globalStorage", "kiro.kiroagent", "profile.json");
const KIRO_SETTINGS_FILE = path.join(os.homedir(), "AppData", "Roaming", "Kiro", "User", "settings.json");
...
async function readJsonFileIfExists(filePath) {
    try {
        ...
        if (error.code !== 'ENOENT') {
            logger.debug(`[Kiro] Failed to read JSON file ${filePath}: ${error.message}`);
        }
        return null;
    }
}

function getErrorResponseSnippet(error, maxLength = 500) {
...
function isInvalidKiroModelError(error) {
    const responseText = `${error?.message || ''} ${getErrorResponseSnippet(error, 1000)}`.toLowerCase();
    return responseText.includes('invalid_model') ||
        responseText.includes('invalid model') ||
        responseText.includes('invalid_model_id') ||
        responseText.includes('invalidmodelid');
}
...
const POLLUTED_TOOL_CALL_TEXT_PATTERN = /\[Called tool [^\]]*\]/g;

function stripPollutedAssistantToolText(content) {
    const text = String(content || '');
    if (!text.includes('[Called tool ')) {
        return text;
    }
    return text
        .replace(POLLUTED_TOOL_CALL_TEXT_PATTERN, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim();
}

function normalizeAssistantHistoryMessage(assistantResponseMessage) {
    if (!assistantResponseMessage || typeof assistantResponseMessage !== 'object') {
        return null;
    }

    const normalized = { ...assistantResponseMessage };
    normalized.content = stripPollutedAssistantToolText(normalized.content);

    if (!Array.isArray(normalized.toolUses) || normalized.toolUses.length === 0) {
        delete normalized.toolUses;
    }

    const hasToolUses = Array.isArray(normalized.toolUses) && normalized.toolUses.length > 0;
    const trimmedContent = String(normalized.content || '').trim();
    if (!hasToolUses && (trimmedContent === '' || trimmedContent === '.')) {
        return null;
    }

    normalized.content = trimmedContent ? normalized.content : '';
    return normalized;
}

function pushAssistantHistoryMessage(history, assistantResponseMessage) {
    const normalized = normalizeAssistantHistoryMessage(assistantResponseMessage);
    if (!normalized) {
        logger.info('[Kiro] Dropping hollow assistant history message');
        return false;
    }
    history.push({ assistantResponseMessage: normalized });
    return true;
}

function stringifyKiroToolValue(value) {
...
function formatToolUseAsText(toolUse) {
    const name = toolUse?.name || 'unknown';
    const id = toolUse?.toolUseId || 'unknown';
    const input = stringifyKiroToolValue(toolUse?.input);
    return `[Tool use: ${name} (${id})${input ? ` input=${input}` : ''}]`;
}

function formatToolResultAsText(toolResult) {
    const id = toolResult?.toolUseId || 'unknown';
    const text = Array.isArray(toolResult?.content)
        ? toolResult.content.map(item => item?.text || '').join('\n')
        : stringifyKiroToolValue(toolResult?.content);
    return `[Tool result for ${id}]${text ? `\n${text}` : ''}`;
}

function appendKiroTextContent(existing, addition) {
...
function downgradeKiroUserToolResults(userInputMessage) {
    const context = userInputMessage?.userInputMessageContext;
    if (!context || typeof context !== 'object' || !Array.isArray(context.toolResults)) {
        return 0;
    }

    let downgraded = 0;
    for (const toolResult of context.toolResults) {
        userInputMessage.content = appendKiroTextContent(
            userInputMessage.content,
            formatToolResultAsText(toolResult)
        );
        downgraded++;
    }
    ...
    removeEmptyObjectField(userInputMessage, 'userInputMessageContext');
    return downgraded;
}

function downgradeKiroAssistantToolUses(assistantResponseMessage) {
    if (!assistantResponseMessage || !Array.isArray(assistantResponseMessage.toolUses)) {
        return 0;
    }

    let downgraded = 0;
    for (const toolUse of assistantResponseMessage.toolUses) {
        assistantResponseMessage.content = appendKiroTextContent(
            assistantResponseMessage.content,
            formatToolUseAsText(toolUse)
        );
        downgraded++;
    }
...
function enforceNoStructuredToolsWithoutConfig(request, hasToolConfig) {
    if (hasToolConfig) {
        return 0;
    }
    ...
    if (Array.isArray(history)) {
        for (const item of history) {
            downgraded += downgradeKiroUserToolResults(item?.userInputMessage);
            downgraded += downgradeKiroAssistantToolUses(item?.assistantResponseMessage);
        }
    }

    const currentUserInputMessage = request?.conversationState?.currentMessage?.userInputMessage;
    downgraded += downgradeKiroUserToolResults(currentUserInputMessage);
    return downgraded;
}
...
function normalizeKiroAuthMethod(authMethod) {
    ...
    if (normalized === 'social') {
        return KIRO_CONSTANTS.AUTH_METHOD_SOCIAL;
    }
    if (normalized === 'idc' || normalized === 'builder-id' || normalized === 'builder_id') {
        return KIRO_CONSTANTS.AUTH_METHOD_IDC;
    }
    return authMethod;
}
...
function normalizeKiroCredentialObject(credentials) {
    ...
    const authValue = normalized.authMethod ?? normalized.auth;
    const normalizedAuthMethod = normalizeKiroAuthMethod(authValue);
    if (normalizedAuthMethod) {
        normalized.authMethod = normalizedAuthMethod;
    }

    const explicitlyKiro2ApiIdc = normalized.auth === 'IdC';
    const isIdcAuth = normalized.authMethod === KIRO_CONSTANTS.AUTH_METHOD_IDC;
    if (explicitlyKiro2ApiIdc && isIdcAuth && (!normalized.clientId || !normalized.clientSecret)) {
        logger.warn('[Kiro Auth] Skipping IdC credentials without clientId/clientSecret.');
        return null;
    }
...
function normalizeKiroCredentials(credentials, sourceLabel = 'credentials') {
    if (Array.isArray(credentials)) {
        for (const item of credentials) {
            const normalized = normalizeKiroCredentialObject(item);
            if (normalized) {
                logger.info(`[Kiro Auth] Loaded first enabled credential from ${sourceLabel} array.`);
                return normalized;
            }
        }
        logger.warn(`[Kiro Auth] No enabled credential found in ${sourceLabel} array.`);
        return null;
    }

    return normalizeKiroCredentialObject(credentials);
}

function updateKiroCredentialsData(existingData, newData, currentRefreshToken) {
    if (!Array.isArray(existingData)) {
        return { ...(existingData || {}), ...newData };
    }

    let updated = false;
    const updatedCredentials = existingData.map(item => {
        if (!item || typeof item !== 'object' || Array.isArray(item)) {
            return item;
        }
        const normalized = normalizeKiroCredentialObject(item);
        const isCurrentCredential = normalized &&
            currentRefreshToken &&
            normalized.refreshToken === currentRefreshToken;
        if (!updated && isCurrentCredential) {
            updated = true;
            return { ...item, ...newData };
        }
        return item;
    ...
    if (!updated) {
        for (let i = 0; i < updatedCredentials.length; i++) {
            const normalized = normalizeKiroCredentialObject(updatedCredentials[i]);
            if (normalized) {
                updatedCredentials[i] = { ...updatedCredentials[i], ...newData };
                updated = true;
                break;
            }
        }
    ...
    if (!updated) {
        updatedCredentials.push(newData);
    }
    ...
    constructor(config = {}) {
        ...
        if (config.KIRO_OAUTH_CREDS_BASE64) {
            try {
                ...
                this.base64Creds = normalizeKiroCredentials(parsedCreds, 'Base64 credentials');
    ...
    async initialize() {
        ...
        const axiosConfig = {
            ...
            headers: {
                ...
                'x-amzn-kiro-agent-mode': KIRO_CONSTANTS.AGENT_TASK_TYPE,
...
async loadCredentials() {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    parsedCredentials = JSON.parse(repaired);
            ...
            return normalizeKiroCredentials(parsedCredentials, filePath);
        } catch (error) {
            if (error.code === 'ENOENT') {
    ...
    try {
            ...
            for (const file of files) {
                if (file.endsWith('.json') && file !== targetFileName) {
                    ...
                    if (credentials) {
                        ...
                        for (const field of ['clientId', 'clientSecret', 'startUrl', 'idcRegion']) {
                            if (credentials[field] !== undefined && credentials[field] !== null && !mergedCredentials[field]) {
                                clientCredentialFields[field] = credentials[field];
                            }
                        }
                        if (Object.keys(clientCredentialFields).length > 0) {
                            Object.assign(mergedCredentials, clientCredentialFields);
                            logger.debug(`[Kiro Auth] Loaded client credential fields from ${file}: ${Object.keys(clientCredentialFields).join(', ')}`);
                        }
                    }
                }
            }
        ...
        if (!this.profileArn) {
            const profile = await readJsonFileIfExists(this.config.KIRO_PROFILE_FILE_PATH || KIRO_PROFILE_FILE);
            if (profile?.arn) {
                this.profileArn = profile.arn;
                logger.info('[Kiro Auth] Loaded profileArn from Kiro profile file.');
            }
        }

        if (!this.region && this.profileArn) {
            const arnParts = String(this.profileArn).split(':');
...
async ensureUsableAccessToken(context = 'request') {
    if (!this.accessToken || this.isTokenExpired()) {
        logger.info(`[Kiro] Token missing or expired before ${context}; refreshing synchronously.`);
        await this.initializeAuth(true);
        return;
    }

    if (this.isExpiryDateNear()) {
        logger.info(`[Kiro] Token is near expiry before ${context}; refreshing synchronously.`);
        try {
            await this.initializeAuth(true);
        } catch (error) {
            if (this.isTokenExpired()) {
                logger.warn(`[Kiro] Near-expiry refresh failed and token is no longer usable before ${context}: ${error.message}`);
                throw error;
            }
            logger.warn(`[Kiro] Near-expiry refresh failed before ${context}; continuing with current token and marking credential for background refresh: ${error.message}`);
            this._markCredentialNeedRefresh(`Near-expiry refresh failed before ${context}`);
        }
    }
}

async getKiroPreferredModelId() {
    if (this.config.KIRO_MODEL_ID) {
        return this.config.KIRO_MODEL_ID;
    }

    const settings = await readJsonFileIfExists(this.config.KIRO_SETTINGS_FILE_PATH || KIRO_SETTINGS_FILE);
    return settings?.['kiroAgent.modelSelection'] ||
        settings?.['kiroAgent.agentModelSelection'] ||
        null;
}

async fetchAvailableModels(forceRefresh = false) {
    const cacheTtlMs = Number(this.config.KIRO_AVAILABLE_MODELS_CACHE_MS || 5 * 60 * 1000);
    const now = Date.now();
    if (!forceRefresh && this.availableModelsCache && now - this.availableModelsCacheAt < cacheTtlMs) {
        return this.availableModelsCache;
    }

    await this.ensureUsableAccessToken('fetchAvailableModels');
    ...
    const axiosConfig = {
        method: 'get',
        url: `${this.managementUrl || KIRO_CONSTANTS.MANAGEMENT_URL.replace("{{region}}", this.region || 'us-east-1')}/ListAvailableModels?${params.toString()}`,
        headers,
        ...
        const models = Array.isArray(response.data?.models)
            ? response.data.models
                .map(model => ({
                    ...
                    name: model.modelName || model.name || model.modelId || model.id,
                    maxInputTokens: model.tokenLimits?.maxInputTokens || model.maxInputTokens,
                    effortLevels: model.effortLevels
                }))
                .filter(model => model.id)
            : [];
        const defaultModelId = response.data?.defaultModel?.modelId || response.data?.defaultModel?.id;
        const result = {
            models,
            defaultModel: defaultModelId
                ? (models.find(model => model.id === defaultModelId) || { id: defaultModelId, name: defaultModelId })
                : undefined
        };
        this.availableModelsCache = result;
        this.availableModelsCacheAt = now;
        if (models.length > 0) {
            logger.info(`[Kiro] Loaded ${models.length} available model(s). Default: ${result.defaultModel?.id || 'none'}`);
        }
        return result;
    } catch (error) {
        const status = error.response?.status;
        const snippet = getErrorResponseSnippet(error);
        logger.warn(`[Kiro] Failed to fetch available models (Status: ${status || 'unknown'}): ${error.message}${snippet ? ` Body: ${snippet}` : ''}`);
        return this.availableModelsCache || { models: [] };
    }
}

async resolveRequestModel(model, options = {}) {
    const requestedModel = model || this.modelName;
    const mappedModel = resolveKiroModelId(requestedModel);
    const preferredModel = await this.getKiroPreferredModelId();
    const mappedPreferred = preferredModel ? resolveKiroModelId(preferredModel) : null;
    const available = await this.fetchAvailableModels(options.forceRefreshModels === true);
    const availableIds = new Set((available.models || []).map(item => item.id));

    if (mappedModel && (availableIds.size === 0 || availableIds.has(mappedModel))) {
        return mappedModel;
    }

    if (mappedPreferred && (availableIds.size === 0 || availableIds.has(mappedPreferred))) {
        logger.warn(`[Kiro] Requested model ${mappedModel} is not available; using Kiro selected model ${mappedPreferred}.`);
        return mappedPreferred;
    }

    if (available.defaultModel?.id) {
        logger.warn(`[Kiro] Requested model ${mappedModel} is not available; using Kiro default model ${available.defaultModel.id}.`);
        return available.defaultModel.id;
    }

    if (available.models?.[0]?.id) {
        logger.warn(`[Kiro] Requested model ${mappedModel} is not available; using first available model ${available.models[0].id}.`);
        return available.models[0].id;
    }
 ...
 */
async saveCredentialsToFile(filePath, newData, currentRefreshToken = this.refreshToken) {
    let existingData = {};
    try {
        const fileContent = await fs.readFile(filePath, 'utf8');
        try {
    ...
    const mergedData = updateKiroCredentialsData(existingData, newData, currentRefreshToken);
    ...
    async _doTokenRefresh(saveCredentialsToFile, tokenFilePath) {
        try {
            ...
            if (response.data && response.data.accessToken) {
                ...
                await saveCredentialsToFile(tokenFilePath, updatedTokenData, currentRefreshToken);
...
${text}`;
        ...
        if (Array.isArray(content)) {
            for (const part of content) {
                if (part.type === 'text') {
                    ...
                    result.text = appendKiroTextContent(result.text, formatToolUseAsText({
     ...
     */
    async buildCodewhispererRequest(messages, model, tools = null, inSystemPrompt = null, thinking = null, options = {}) {
        ...
        const codewhispererModel = options.resolvedModelId || await this.resolveRequestModel(model, {
            forceRefreshModels: options.forceRefreshModels
        });
        ...
        if (tools && Array.isArray(tools) && tools.length > 0) {
            ...
            if (filteredTools.length === 0) {
                logger.info('[Kiro] All tools were filtered out; sending request without tools');
            } else {
                let truncatedCount = 0;
                const kiroTools = filteredTools.map(tool => {
                        const kiroToolName = toolNameMaps.toKiroName(tool.name);
                        let desc = normalizeKiroToolDescription(tool.description, kiroToolName);
                        const originalLength = desc.length;
                        
                        if (desc.length > KIRO_MAX_TOOL_DESCRIPTION_LENGTH) {
                            desc = desc.substring(0, KIRO_MAX_TOOL_DESCRIPTION_LENGTH) + "...";
                            truncatedCount++;
                        ...
                        return {
                            toolSpecification: {
                                ...
                                description: desc,
                                inputSchema: {
                                    json: sanitizeKiroToolSchema(tool.input_schema)
                                }
                            }
                        };
                    });
                ...
                if (truncatedCount > 0) {
                    logger.info(`[Kiro] Truncated ${truncatedCount} tool description(s) to max ${KIRO_MAX_TOOL_DESCRIPTION_LENGTH} chars`);
                }
        ...
        const hasToolConfig = Array.isArray(toolsContext.tools) && toolsContext.tools.length > 0;
        ...
        for (let i = startIndex; i < processedMessages.length - 1; i++) {
            ...
            if (message.role === 'user') {
                    ...
                    for (const part of message.content) {
                        if (part.type === 'text') {
                            ...
                            if (hasToolConfig) {
                                ...
                                userInputMessage.content = appendKiroTextContent(userInputMessage.content, formatToolResultAsText(toolResult));
                            }
                    ...
                    for (const part of message.content) {
                        if (part.type === 'text') {
                            ...
                            if (hasToolConfig) {
                                ...
                                assistantResponseMessage.content = appendKiroTextContent(assistantResponseMessage.content, formatToolUseAsText({
                                    ...toolUse,
                                    name: part.name
                                }));
                            }
                ...
                pushAssistantHistoryMessage(history, assistantResponseMessage);
            }
        }
            ...
            if (Array.isArray(currentMessage.content)) {
                for (const part of currentMessage.content) {
                    if (part.type === 'text') {
                        ...
                        if (hasToolConfig) {
                            assistantResponseMessage.toolUses.push(toolUse);
                        } else {
                            assistantResponseMessage.content = appendKiroTextContent(assistantResponseMessage.content, formatToolUseAsText({
                                ...toolUse,
                                name: part.name
                ...
                for (const part of currentMessage.content) {
                    if (part.type === 'text') {
                        ...
                        if (hasToolConfig) {
                            ...
                            currentContent = appendKiroTextContent(currentContent, formatToolResultAsText(toolResult));
                        }
                    } else if (part.type === 'tool_use') {
                        const toolUse = {
                            input: this._sanitizeToolInput(part.input),
                        ...
                        if (hasToolConfig) {
                            ...
                            currentContent = appendKiroTextContent(currentContent, formatToolUseAsText({
                                ...toolUse,
                                name: part.name
                            }));
                        }
        ...
        const request = {
            conversationState: {
                ...
                agentTaskType: KIRO_CONSTANTS.AGENT_TASK_TYPE,
        ...
        const downgradedToolBlocks = enforceNoStructuredToolsWithoutConfig(request, hasToolConfig);
        if (downgradedToolBlocks > 0) {
     ...
     * 调用 API 并处理错误重试
     */
    async callApi(method, model, body, isRetry = false, retryCount = 0, resolvedModelId = null) {
        ...
        try {
            ...
            if (isNetworkError && retryCount < maxRetries) {
                ...
                return this.callApi(method, model, body, isRetry, retryCount + 1, resolvedModelId);
            }

            if (status === 400 && !resolvedModelId && isInvalidKiroModelError(error)) {
                const available = await this.fetchAvailableModels(true);
                const fallbackModel = available.defaultModel?.id || available.models?.[0]?.id;
                const attemptedModel = requestData.conversationState?.currentMessage?.userInputMessage?.modelId;
                if (fallbackModel && fallbackModel !== attemptedModel) {
                    logger.warn(`[Kiro] Invalid model ${attemptedModel} rejected by upstream. Retrying once with Kiro default model ${fallbackModel}.`);
                    return this.callApi(method, model, body, isRetry, retryCount, fallbackModel);
                }
            }

            const responseSnippet = getErrorResponseSnippet(error);
            if (responseSnippet) {
                logger.error('[Kiro] API response body:', responseSnippet);
            }
    ...
    async generateContent(model, requestBody) {
        ...
        await this.ensureUsableAccessToken('generateContent');
        ...
        try {
            ...
            if (isWhitespaceOnly(responseText) && (!toolCalls || toolCalls.length === 0)) {
                const emptyResponseError = createKiroEmptyResponseError(model, inputTokens);
                logger.warn(`[Kiro] Empty unary response detected for model=${model}, input_tokens=${inputTokens}`);
                this._markCredentialNeedRefresh('Empty Kiro unary response', emptyResponseError);
            ...
            logger.error(`[Kiro] Error in generateContent: ${error.message}`);
            throw error;
        }
    }
     ...
     */
    async * streamApiReal(method, model, body, isRetry = false, retryCount = 0, resolvedModelId = null) {
        if (!this.isInitialized) await this.initialize();
        const maxRetries = this.config.REQUEST_MAX_RETRIES || 3;
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                buffer = remaining;
                if (ignoredInChunk > 0) {
                    yield { type: 'ignoredEvents', count: ignoredInChunk, samples: ignoredSamplesInChunk };
                }
            ...
            if (isNetworkError && retryCount < maxRetries) {
                ...
                await new Promise(resolve => setTimeout(resolve, delay));
                yield* this.streamApiReal(method, model, body, isRetry, retryCount + 1, resolvedModelId);
                return;
            }
            ...
            if (status === 400 && !resolvedModelId && isInvalidKiroModelError(error)) {
                ...
                const attemptedModel = requestData.conversationState?.currentMessage?.userInputMessage?.modelId;
                if (fallbackModel && fallbackModel !== attemptedModel) {
                    logger.warn(`[Kiro] Invalid stream model ${attemptedModel} rejected by upstream. Retrying once with Kiro default model ${fallbackModel}.`);
                    yield* this.streamApiReal(method, model, body, isRetry, retryCount, fallbackModel);
                    return;
                }
            }
            ...
            const responseSnippet = getErrorResponseSnippet(error);
            if (responseSnippet) {
                logger.error('[Kiro] Stream API response body:', responseSnippet);
            }
    ...
    async streamApi(method, model, body, isRetry = false, retryCount = 0) {
        try {
            ...
            logger.error(`[Kiro] Error calling API: ${error.message}`);
            throw error;
        }
    }
    ...
    async * generateContentStream(model, requestBody) {
        ...
        await this.ensureUsableAccessToken('generateContentStream');
        ...
        let streamEstimatedInputTokens = 0;
        ...
        function createMessageStartEvent() {
            return {
                ...
                message: {
                    ...
                    model: model,
                    usage: {
                        input_tokens: streamEstimatedInputTokens,
                        output_tokens: 0,
        ...
        try {
            ...
            const activeToolCalls = new Map(); // toolUseId -> current structured tool call
            let ignoredUpstreamEventCount = 0;
            let sawIgnoredUpstreamActivity = false;
            let pendingIgnoredActivityPing = false;
            const ignoredUpstreamSamples = [];

            function* flushPendingIgnoredActivityPing() {
                if (!pendingIgnoredActivityPing) return;
                pendingIgnoredActivityPing = false;
                yield* pushPing();
            }

            const estimatedInputTokens = this.estimateInputTokens(requestBody);
            streamEstimatedInputTokens = estimatedInputTokens;

            const finishToolCall = (toolCall, toolEvents) => {
                if (!toolCall) {
                    return;
                }
                ...
                try {
                    ...
                    throw createKiroIncompleteToolUseError(toolCall, error.message);
                }
                ...
                if (Array.isArray(toolCall.pendingEvents) && toolCall.pendingEvents.length > 0) {
                    toolEvents.push(...stopBlock(streamState.textBlockIndex));
                    toolEvents.push(...toolCall.pendingEvents);
                }
                ...
                activeToolCalls.delete(toolCall.toolUseId);
                if (currentToolCall?.toolUseId === toolCall.toolUseId) {
            ...
            for await (const event of this.streamApiReal('', finalModel, requestBody)) {
                if (event.type === 'contextUsage' && event.contextUsagePercentage) {
                    ...
                    if (isWhitespaceOnly(event.content)) {
                        continue;
                    }
                    yield* flushPendingIgnoredActivityPing();
                    ...
                    if (tc.name && tc.toolUseId) {
                        currentToolCall = activeToolCalls.get(tc.toolUseId);
                        if (!currentToolCall) {
                            ...
                            activeToolCalls.set(tc.toolUseId, currentToolCall);
                        }
                        currentToolCall.input += tc.input || '';

                        // 工具参数必须等到 toolUseStop 且 JSON 完整后才对客户端可见。
                        // 否则上游中断时，Claude Code 会收到半截 input_json_delta 并解析失败。
                        if (tc.input) {
                            const blockIndex = toolUseBlockIndexes.get(tc.toolUseId);
                            if (blockIndex != null) {
                                currentToolCall.pendingEvents.push({
                                    type: "content_block_delta",
                                    index: blockIndex,
                                    delta: {
                        ...
                        if (tc.stop && currentToolCall) {
                            finishToolCall(currentToolCall, toolEvents);
                        }
                    }
                    ...
                    const targetToolCall = event.toolUseId && activeToolCalls.has(event.toolUseId)
                        ? activeToolCalls.get(event.toolUseId)
                        : currentToolCall;
                    ...
                    if (targetToolCall) {
                        targetToolCall.input += inputDelta;
                        currentToolCall = targetToolCall;
                        const blockIndex = toolUseBlockIndexes.get(targetToolCall.toolUseId);
                        if (blockIndex != null && inputDelta) {
                            targetToolCall.pendingEvents.push({
                                type: "content_block_delta",
                                index: blockIndex,
                                delta: {
                                    type: "input_json_delta",
                    ...
                    const targetToolCall = event.toolUseId && activeToolCalls.has(event.toolUseId)
                        ? activeToolCalls.get(event.toolUseId)
                        : currentToolCall;
                    if (targetToolCall && event.stop) {
                        const toolEvents = [];
                        finishToolCall(targetToolCall, toolEvents);
                        yield* flushPendingIgnoredActivityPing();
                        yield* pushEvents(toolEvents);
                    }
                } else if (event.type === 'ignoredEvents') {
                    ignoredUpstreamEventCount += event.count || 0;
                    sawIgnoredUpstreamActivity = true;
                    pendingIgnoredActivityPing = true;
                    if (Array.isArray(event.samples)) {
                        for (const s of event.samples) {
                            if (ignoredUpstreamSamples.length < 20) ignoredUpstreamSamples.push(s);
                        }
                    }
                }
            }
            ...
            const hasSubstantiveOutput = !isWhitespaceOnly(totalContent) ||
                streamState.hasVisibleText ||
                streamState.hasThinkingContent ||
                toolCalls.length > 0;
            if (!hasSubstantiveOutput) {
                logger.warn(`[Kiro Stream] Empty upstream response detected for model=${model}, input_tokens=${estimatedInputTokens}, ignored_events=${ignoredUpstreamEventCount}`);
                // 诊断日志：转储被忽略的上游事件原文与请求体，便于定位 ignored_events 空响应
                try {
                    if (ignoredUpstreamSamples.length > 0) {
                        logger.warn(`[Kiro Stream][Diag] Ignored upstream event samples (${ignoredUpstreamSamples.length}): ${JSON.stringify(ignoredUpstreamSamples)}`);
                    }
                    const reqDump = JSON.stringify(requestBody);
                    logger.warn(`[Kiro Stream][Diag] Failing request body (len=${reqDump.length}): ${reqDump.length > 8000 ? reqDump.slice(0, 8000) + '...(truncated)' : reqDump}`);
                } catch (diagErr) {
                    logger.warn(`[Kiro Stream][Diag] Failed to dump diagnostics: ${diagErr.message}`);
                }
                if (sawIgnoredUpstreamActivity) {
                    const emptyResponseError = createKiroEmptyResponseError(model, estimatedInputTokens, { reason: 'ignored_events' });
                    logger.warn('[Kiro Stream] Only Kiro side-channel activity was received; rejecting stream before empty success response.');
                    throw emptyResponseError;
                } else {
                    const emptyResponseError = createKiroEmptyResponseError(model, estimatedInputTokens, { reason: 'empty' });
                    this._markCredentialNeedRefresh('Empty Kiro stream response', emptyResponseError);
                    throw emptyResponseError;
                }
            }
            ...
            yield* pushEvents([{ type: "message_stop" }]);

        } catch (error) {
            logger.error(`[Kiro] Error in streaming generation: ${error.message}`);
            throw error;
        }
    }
