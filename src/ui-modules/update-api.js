...
function getUpdateRepository() {
    return process.env.AICLIENT2API_UPDATE_REPO || '';
}
...
async function getVersionsFromGitHub(limit = 10) {
    ...
    if (!repo) {
        logger.warn('[Update] AICLIENT2API_UPDATE_REPO is not configured; skipping GitHub API update check');
        return [];
    }
...
async function performTarballUpdate(localVersion, latestTag) {
    const repo = getUpdateRepository();
    if (!repo) {
        throw new Error('AICLIENT2API_UPDATE_REPO is not configured; tarball update is unavailable');
    }

    const tarballCandidates = buildTarballCandidates(repo, latestTag);
