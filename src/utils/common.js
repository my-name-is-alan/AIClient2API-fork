...
const CLAUDE_CODE_FILE_WRITE_GUIDANCE_MARKER = '[AIClient2API file-write safety guidance]';
const CLAUDE_CODE_FILE_WRITE_GUIDANCE = `${CLAUDE_CODE_FILE_WRITE_GUIDANCE_MARKER}
When creating or modifying long text files, Markdown documents, JSON, code files, logs, configs, or any file content likely to exceed 8KB, do not send the full file content in one Write tool call. Use a chunked write strategy instead:
1. For new files, first create the file with a short heading, skeleton, or first small section.
2. Append or replace subsequent sections with Edit in batches around 4KB to 8KB each.
3. Split large Markdown, code, JSON, generated documents, and logs by section or logical block.
4. Keep every tool call's JSON input compact; avoid embedding an entire large file in a single tool input.
5. After all chunks are written, read or inspect the target file to verify the content is complete.`;
...
function createIncompleteClaudeToolUseError(message) {
    const error = new Error(message);
    error.status = 502;
    error.code = 'CLAUDE_TOOL_USE_INCOMPLETE';
    return error;
}

function createClaudeToolUseBoundaryBuffer(enabled) {
    const pendingToolBlocks = new Map();

    const makeKey = (chunk) => String(chunk?.index ?? 0);

    const ensureValidJson = (state) => {
        const rawJson = state.partialJson || JSON.stringify(state.start.content_block?.input || {});
        try {
            return JSON.parse(rawJson);
        } catch (error) {
            const toolName = state.start.content_block?.name || `index ${state.start.index ?? 0}`;
            throw createIncompleteClaudeToolUseError(
                `[Stream] Incomplete Claude tool_use JSON for ${toolName}: ${error.message}`
            );
        }
    };
    ...
    return {
        process(chunk) {
            ...
            if (chunk.type === 'content_block_start' && chunk.content_block?.type === 'tool_use') {
                pendingToolBlocks.set(makeKey(chunk), {
                    start: chunk,
                    partialJson: ''
                });
                return [];
            ...
            if (chunk.type === 'content_block_delta' && chunk.delta?.type === 'input_json_delta') {
                ...
                const state = pendingToolBlocks.get(key);
                if (!state) {
                    throw createIncompleteClaudeToolUseError(
                        `[Stream] Claude input_json_delta arrived without a matching tool_use start for index ${chunk.index ?? 0}`
                    );
                }
                state.partialJson += chunk.delta.partial_json || '';
                return [];
            }
            ...
            if (chunk.type === 'content_block_stop') {
                ...
                const parsedInput = ensureValidJson(state);
                pendingToolBlocks.delete(key);
                const completeJson = JSON.stringify(parsedInput);
                return [
                    {
                        ...state.start,
                        content_block: {
                            ...state.start.content_block,
                            input: {}
                        }
                    },
                    {
                        type: 'content_block_delta',
                        index: state.start.index ?? chunk.index ?? 0,
                        delta: {
            ...
            if (pendingToolBlocks.size > 0 && (chunk.type === 'message_delta' || chunk.type === 'message_stop')) {
                throw createIncompleteClaudeToolUseError(
                    '[Stream] Claude stream ended before completing a pending tool_use block'
                );
            }
        ...
        assertDrained() {
            if (enabled && pendingToolBlocks.size > 0) {
                throw createIncompleteClaudeToolUseError(
                    '[Stream] Claude stream ended with an incomplete tool_use block'
                );
            }
        }
    };
...
export async function handleStreamRequest(res, service, model, requestBody, fromProvider, toProvider, PROMPT_LOG_MODE, PROMPT_LOG_FILENAME, providerPoolManager, pooluuid, customName, retryContext = null) {
    ...
    try {
        ...
        const claudeToolUseBuffer = createClaudeToolUseBoundaryBuffer(
        ...
        for await (const nativeChunk of nativeStream) {
            ...
            if (hookRequestId) {
                try {
                    ...
                    logger.warn(`[Stream Hook] onStreamChunk failed for request ${hookRequestId}: ${e.message}`);
                }
            }
            ...
            for (const candidateChunk of chunksToSend) {
                const bufferedChunks = claudeToolUseBuffer.process(candidateChunk);
        ...
        claudeToolUseBuffer.assertDrained();
            ...
            if (!res.writableEnded) {
                try {
                    if (clientProtocol === MODEL_PROTOCOL_PREFIX.OPENAI) {
                        // OpenAI chat-completions streams use [DONE] as the SSE completion sentinel.
                        // A finish_reason chunk is not a replacement for it; many clients wait for [DONE].
                        res.write('data: [DONE]\n\n');
                        hasMessageStop = true;
    ...
    try{
        ...
        if (hookRequestId) {
            try {
                ...
                logger.warn(`[Unary Hook] onUnaryResponse failed for request ${hookRequestId}: ${e.message}`);
            }
        }
...
export async function handleContentGenerationRequest(req, res, service, endpointType, CONFIG, PROMPT_LOG_FILENAME, providerPoolManager, pooluuid, requestPath = null) {
    ...
    processedRequestBody = _applyClaudeCodeFileWriteGuidance(CONFIG, processedRequestBody, toProvider);
...
function _applyClaudeCodeFileWriteGuidance(config, requestBody, toProvider) {
    if (config?.CLAUDE_CODE_FILE_WRITE_GUIDANCE === false) {
        return requestBody;
    }

    if (getProtocolPrefix(toProvider) !== MODEL_PROTOCOL_PREFIX.CLAUDE) {
        return requestBody;
    }

    const appendGuidance = (text) => {
        if (typeof text !== 'string' || text.length === 0) {
            return CLAUDE_CODE_FILE_WRITE_GUIDANCE;
        }
        if (text.includes(CLAUDE_CODE_FILE_WRITE_GUIDANCE_MARKER)) {
            return text;
        }
        return `${text}\n\n${CLAUDE_CODE_FILE_WRITE_GUIDANCE}`;
    };

    if (Array.isArray(requestBody.system)) {
        const textBlockIndex = requestBody.system.findIndex(block => block && block.type === 'text' && typeof block.text === 'string');
        if (textBlockIndex !== -1) {
            requestBody.system[textBlockIndex] = {
                ...requestBody.system[textBlockIndex],
                text: appendGuidance(requestBody.system[textBlockIndex].text)
            };
        } else {
            requestBody.system.push({ type: 'text', text: CLAUDE_CODE_FILE_WRITE_GUIDANCE });
        }
        return requestBody;
    }

    if (requestBody.system && typeof requestBody.system === 'object') {
        requestBody.system = appendGuidance(JSON.stringify(requestBody.system));
        return requestBody;
    }

    requestBody.system = appendGuidance(requestBody.system);
    return requestBody;
}
