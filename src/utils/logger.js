...
function redactString(value) {
    return String(value)
        ...
        .replace(/(authorization["']?\s*[:=]\s*["']?\s*Bearer\s+)[^"',\s}]+/gi, `$1${REDACTED_VALUE}`)
        .replace(/((?:access|refresh|id)[_-]?token["']?\s*[:=]\s*["']?)[^"',\s}&]+/gi, `$1${REDACTED_VALUE}`)
        .replace(/(client[_-]?secret["']?\s*[:=]\s*["']?)[^"',\s}&]+/gi, `$1${REDACTED_VALUE}`)
        .replace(/((?:api[_-]?key|x-api-key|x-goog-api-key)["']?\s*[:=]\s*["']?)[^"',\s}&]+/gi, `$1${REDACTED_VALUE}`)
        .replace(/([?&](?:access_token|refresh_token|id_token|token|api_key|key|client_secret)=)[^&#\s]+/gi, (match, prefix) => {
            const separatorIndex = match.indexOf('&', prefix.length);
            if (separatorIndex === -1) {
                return `${prefix}${REDACTED_VALUE}`;
            }
            return `${prefix}${REDACTED_VALUE}${match.slice(separatorIndex)}`;
        });
}
...
function sanitizeForLog(value, seen = new WeakSet()) {
    ...
    for (const [key, childValue] of Object.entries(value)) {
        sanitized[key] = isSensitiveKey(key)
            ? REDACTED_VALUE
            : sanitizeForLog(childValue, seen);
    }
    return sanitized;
}
...
class Logger {
    ...
    formatMessage(level, args, requestId) {
        ...
        const message = args.map(arg => {
            const sanitizedArg = sanitizeForLog(arg);
            ...
            return redactString(String(sanitizedArg));
        }).join(' ');
