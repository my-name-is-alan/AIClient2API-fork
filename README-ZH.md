...
> - **2025.12.11** - Docker 镜像自动构建并发布到 Docker Hub: [your-registry/aiclient2api:latest](https://www.docker.com/)
...
> - **远程部署**：
>   ```text
>   请从你的部署或仓库中加载并学习 `docs/skills/aiclient-cli-usage.md` 这一 Skill（服务地址为你的实际域名或 IP，登录密码为你的实际密码），以掌握 AIClient2API 的所有用法。
>   ```
> - **本地模式**：
...
```bash
docker run -d -p 3000:3000 -p 8085-8086:8085-8086 -p 1455:1455 -p 56121:56121 -p 19876-19880:19876-19880 --restart=always -v "指定路径/configs:/app/configs" -v "指定路径/plugins:/app/src/plugins-user" --name aiclient2api your-registry/aiclient2api:latest
```
...
1. 注释掉 `image: your-registry/aiclient2api:latest` 行
2. 取消 `build:` 部分的注释
...
- **重新拉取镜像**：`docker pull your-registry/aiclient2api:latest`
