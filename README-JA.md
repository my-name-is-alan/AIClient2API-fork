...
[**🔧 OpenClaw 設定**](./docs/OPENCLAW_CONFIG_GUIDE-JA.md) | [中文](./README-ZH.md) | [English](./README.md) | [**👉 日本語**](./README-JA.md) | [**📚 完全ドキュメント**](./docs/)
...
> - **2025.12.11** - Dockerイメージが自動的にビルドされ、Docker Hubで公開されました: [your-registry/aiclient2api:latest](https://www.docker.com/)
...
> - **リモートデプロイ**：
>   ```text
>   デプロイ環境またはリポジトリ内の `docs/skills/aiclient-cli-usage.md` にある Skill をロードして学習し（サービスアドレス：実際のドメインまたは IP、ログインパスワード：実際のパスワード）、AIClient2API のすべての使用方法をマスターしてください。
>   ```
> - **ローカルモード**：
...
- [💡 コアアドバンテージ](#-コアアドバンテージ)
- [🚀 クイックスタート](#-クイックスタート)
  - [🐳 Docker デプロイ](https://www.docker.com/)
  - [📋 コア機能](#-コア機能)
- [🔐 認証設定ガイド](#-認証設定ガイド)
...
```bash
docker run -d -p 3000:3000 -p 8085-8086:8085-8086 -p 1455:1455 -p 56121:56121 -p 19876-19880:19876-19880 --restart=always -v "指定パス/configs:/app/configs" -v "指定パス/plugins:/app/src/plugins-user" --name aiclient2api your-registry/aiclient2api:latest
```
...
1. `image: your-registry/aiclient2api:latest` 行をコメントアウト
...
- **イメージを再取得**：`docker pull your-registry/aiclient2api:latest`
